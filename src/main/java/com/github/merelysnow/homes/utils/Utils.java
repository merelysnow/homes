package com.github.merelysnow.homes.utils;
...
public class Utils {
    ...
    public static void listPublicHomes(Player player, User user) {
        HashMap<String, Homes> homes = user.getHomes();

        Set<Homes> publicHomes = new HashSet<>();
        ...
        ComponentBuilder publicComponent = new ComponentBuilder("Homes: ").color(ChatColor.YELLOW);
        format(user, publicHomes, publicComponent);

        player.spigot().sendMessage(publicComponent.create());
    }

    private static void format(User user, Set<Homes> homes, ComponentBuilder component) {
        if (homes.isEmpty()) {
            component.append("Nenhuma home definida.").color(ChatColor.WHITE);
            return;
        }

        Iterator<Homes> iterator = homes.iterator();

        while (iterator.hasNext()) {
            Homes home = iterator.next();

            component.append(home.getName());
            component.color(ChatColor.WHITE);
            component.event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.fromLegacyText("Clique para teleportar-se!")));
            component.event(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/home " + user.getName() + ":" + home.getName()));
            component.append(iterator.hasNext() ? ", " : ".");
    ...
    private static final PluginLoader loader = HomesPlugin.plugin.getPluginLoader();
